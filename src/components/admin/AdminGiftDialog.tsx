...
import { KEY_TYPES, KEY_DISPLAY, KeyType } from "@/hooks/data/useKeys";
...
export function AdminGiftDialog({ open, onClose, user }: AdminGiftDialogProps) {
  ...
  const [keyType, setKeyType] = useState<KeyType>("golden");
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Grant points or keys to this user. They will receive a notification.
          </DialogDescription>
        </DialogHeader>
                  ...
                  {KEY_TYPES.map(kt => (
                    <SelectItem key={kt} value={kt}>
                      <span className="flex items-center gap-2">
                        <span>{KEY_DISPLAY[kt].emoji}</span>
                        <span className={KEY_DISPLAY[kt].color}>{KEY_DISPLAY[kt].name} Key</span>
                      </span>
                    </SelectItem>
                  ))}
                </SelectContent>
